...
class frame:
    ...
    def __init__(nodes: np.array, element_list: list, xsection_list: list, constraints: list = [[]]):
        ...
        frame.fixed_dof = np.zeros((N_nodes, 6))
        for constraint in constraints: frame.fixed_dof[constraint[0]] = constraint[1:7]

        # Create list of elements
        frame.elements = []
        for el in element_list: frame.elements.append(element(el[0], el[1], el[2], el[3]))


class element:

    def __init__(node_a: int, node_b: int, xsec: list, z_vec = []):
        element.a = node_a
        element.b = node_b
        element.xsec = xsec
        element.L = np.linalg.norm(nodes[element[1], 0:3] - nodes[element[0], 0:3])

        # Calculate coord transform matrix gamma
        x_vec = (node_b[0:3] - node_a[0:3]) / element.L
        if len(z_vec) == 0:
            if x_vec[0] < 0.95: z_vec = np.cross(x_vec, np.array([1, 0, 0]))
            else: z_vec = np.cross(x_vec, np.array([0, 1, 0]))
        z_vec = z_vec / np.linalg.norm(z_vec)
        element.gamma = np.vstack((x_vec, np.cross(z_vec, x_vec), z_vec))
